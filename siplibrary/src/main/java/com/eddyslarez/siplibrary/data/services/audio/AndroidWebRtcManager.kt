...
import com.eddyslarez.siplibrary.data.services.ia.OpenAIRealtimeTranslationClient
...
class AndroidWebRtcManager(
    ...
    private var translationClient: OpenAIRealtimeTranslationClient? = null
    private var translationConfig: TranslationConfig = TranslationConfig()
    private var isTranslationEnabled = false
    // Buffer de audio para traducción
    private val translationAudioBuffer = mutableListOf<ByteArray>()
    private val translationBufferLock = Mutex()


    ////////traducion///


    fun setTranslationConfig(config: TranslationConfig) {
        this.translationConfig = config
        this.isTranslationEnabled = config.isEnabled

        if (isTranslationEnabled && openAiApiKey != null) {
            setupTranslationClient()
        } else {
            disposeTranslationClient()
        }
    }

    private fun setupTranslationClient() {
        ...
        coroutineScope.launch {
            try {
                // Crear nuevo cliente de traducción
                translationClient = OpenAIRealtimeTranslationClient(openAiApiKey, translationConfig)

                // Configurar listeners
                translationClient?.setConnectionStateListener { connected ->
                    log.d(TAG) { "Translation client connection: $connected" }
                }

                translationClient?.setErrorListener { error ->
                    log.e(TAG) { "Translation error: $error" }
                }

                translationClient?.setTranslationListener { original, translated ->
                    log.d(TAG) { "Translation: '$original' -> '$translated'" }
                    // Aquí podrías notificar a la UI si es necesario
                }

                translationClient?.setSpeechDetectionListener { isActive ->
                    log.d(TAG) { "Translation speech detection: $isActive" }
                }

                // Conectar
                val connected = translationClient?.connect() ?: false
                log.d(TAG) { "Translation client connected: $connected" }

            } catch (e: Exception) {
                log.e(TAG) { "Error setting up translation client: ${e.message}" }
            }
        }
    }

    private fun disposeTranslationClient() {
        coroutineScope.launch {
            try {
                translationClient?.disconnect()
                translationClient = null
            } catch (e: Exception) {
                log.e(TAG) { "Error disposing translation client: ${e.message}" }
            }
        }
    }
...
//            )
    ...
    fun getTranslationConfig(): TranslationConfig = translationConfig

    fun getTranslationStatus(): String {
        return buildString {
            appendLine("=== TRANSLATION STATUS ===")
            appendLine("Enabled: $isTranslationEnabled")
            appendLine("Config: ${translationConfig.languagePair.displayName}")
            appendLine("Mode: ${translationConfig.translationMode}")
            appendLine("Client Connected: ${translationClient != null}")
            appendLine("Voice: ${translationConfig.voiceSettings.voice}")
        }
    }
...
//    }
    ...
    private fun setupAudioInterceptionWithSink(audioTrack: AudioTrack) {
        val audioSink = object : AudioTrackSink {
            override fun onData(
                ...
                coroutineScope.launch {
                    ...
                    // NUEVO: Procesar para traducción (si está habilitada)
                    if (isTranslationEnabled && translationClient != null) {
                        processAudioForTranslation(data, sampleRate)
                    }
                }
            }
        }
        ...
        log.d(TAG) { "Audio sink added for OpenAI and Translation processing" }
    }

    @RequiresApi(Build.VERSION_CODES.O)
    private suspend fun processAudioForTranslation(audioData: ByteArray, sampleRate: Int) {
        try {
            translationBufferLock.withLock {
                translationAudioBuffer.add(audioData)

                val totalBufferSize = translationAudioBuffer.sumOf { it.size }
                val requiredBufferSize = (sampleRate * 2 * BUFFER_SIZE_MS) / 1000 // 100ms worth of audio

                if (totalBufferSize >= requiredBufferSize) {
                    val combinedBuffer = translationAudioBuffer.fold(ByteArray(0)) { acc, data -> acc + data }
            ...
            log.e(TAG) { "Error processing audio for translation: ${e.message}" }
        }
    }
