    ...
    fun buildRegisterMessage(
        ...
        log.d(tag = TAG) {
            "Building REGISTER message:" +
                    ...
                    "\nReason: ${if (isAppInBackground) "PUSH MODE" else "FOREGROUND MODE"}"
        }
        ...
        if (isAppInBackground) {
            log.d(tag = TAG) { "Adding push notification parameters to Contact header for ${accountInfo.username}@${accountInfo.domain}" }
            builder.append(";pn-prid=${accountInfo.token};pn-provider=${accountInfo.provider}")
        } else {
            log.d(tag = TAG) { "Building Contact header WITHOUT push notification parameters for ${accountInfo.username}@${accountInfo.domain}" }
        }
        ...
        // Log del mensaje final para verificar MEJORADO
        log.d(tag = TAG) {
            "Final REGISTER message for ${accountInfo.username}@${accountInfo.domain}:" +
                    "\n- Contact header contains push params: ${finalMessage.contains("pn-prid")}" +
                    "\n- Mode: ${if (isAppInBackground) "PUSH (with token)" else "FOREGROUND (no token)"}" +
                    "\n- User-Agent: ${accountInfo.userAgent}"
        }
